...
module.exports = (param, type, allowedEmpty = true, error = errorType.INVALID_ARGUMENTS, msg = '') => {
    ...
    if (supportedConstructor.has(type)) {
        ...
        if (param.constructor !== type) {
            throw new Exception(msg || `Invalid arguments: require ${typeIs(new type())}, but get ${typeIs(param)}`, error);
        }
    ...
    if (type.constructor === Object) {
        if (param.constructor !== Object) {
            throw new Exception(msg || `Invalid arguments: require object, but get ${typeIs(param)}`, error);
        }
        ...
        for (const key in type) {
            ...
            if (supportedConstructor.has(type[key])) {
                if (param[key] && param[key].constructor !== type[key]) {
                    throw new Exception(`Invalid arguments: require object key "${key}" is ${typeIs(new type[key]())}, but get ${typeIs(param[key])}`, errorType.INVALID_ARGUMENTS);
                }
                ...
                if (supportedConstructor.has(type[key].type)) {
                    ...
                    if (!empty.has(param[key])) {
                        ...
                        if (param[key].constructor !== type[key].type) {
                            throw new Exception(type[key].msg || `Invalid arguments: require object key "${key}" is ${typeIs(new type[key].type())}, but get ${typeIs(param[key])}`, type[key].error || errorType.INVALID_ARGUMENTS);
                        }
