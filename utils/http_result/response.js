...
module.exports = _response => new class resReult {
    ...
    _resSend(_result) {
        ...
        trace(traceModule.default, { traceId: this.request.headers['x-b3-traceid'], spanId: this.request.headers['x-b3-spanid'], parentSpanId: this.request.headers['x-b3-parentspanid'] }).info(`[${addressIpv4}(${this.request.method}): ${this.request.protocol}://${this.request.get('host')}${this.request.originalUrl}] request parameter :${_datas || ' no parameter'} , response result : ${JSON.stringify(_result)}`);
